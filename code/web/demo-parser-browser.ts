...
export class DemoParser {
  public parseDemo(demoData: Uint8Array) {
...
class BitStreamReader {
  ...
  private currentBits: number = 0; // Add initializer
  private bitIdx: number = 0; // Add initializer
...
class Q3DemoParser {
  private demoData: Uint8Array;

  constructor(demoData: Uint8Array) {
  ...
  public static getRawConfigStrings(demoData: Uint8Array): any[] | null {
    return new Q3DemoParser(demoData).parseConfig();
  }

  public static getFriendlyConfig(demoData: Uint8Array): any | null {
    ...
    if (conf[Q3Const.Q3_DEMO_CFG_FIELD_CLIENT]) {
      result.client = Q3Utils.splitConfig(
        conf[Q3Const.Q3_DEMO_CFG_FIELD_CLIENT],
      );
    ...
    if (conf[Q3Const.Q3_DEMO_CFG_FIELD_PLAYER]) {
      result.player = Q3Utils.splitConfig(
        conf[Q3Const.Q3_DEMO_CFG_FIELD_PLAYER],
      );
    }
...
class Q3MessageStream {
  ...
  public nextMessage(): Q3DemoMessage | null {
    ...
    if (msgLength < 0 || msgLength > Q3_MESSAGE_MAX_SIZE) {
      throw new Error(
        `Demo file is corrupted, wrong message length: ${msgLength}`,
      );
    }

    // Read message data
    const msgData = this.demoData.slice(
      this.position,
...
class Q3Utils {
  ...
  public static unpack(data: Uint8Array): number[] {
    const result: number[] = [];
